...
export const LanguageCard: React.FC<LanguageCardProps> = ({ lang, maxWeight }) => {
  const weightValue = parseFloat(lang.weight) || 0;
  const weightPct =
    maxWeight > 0 ? Math.max(0, Math.min(100, (weightValue / maxWeight) * 100)) : 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            {hasTokenScoring ? (
              <Check
                sx={(theme) => ({ fontSize: '1.1rem', color: theme.palette.status.success })}
              />
            ) : (
              <Close
                sx={(theme) => ({ fontSize: '1.1rem', color: theme.palette.status.error })}
              />
            )}
          </Box>
        </Tooltip>
      ...
      <Typography
        sx={(theme) => ({
          fontSize: '0.85rem',
          color: lang.language ? 'text.secondary' : alpha(theme.palette.common.white, 0.3),
          flex: 1,
        })}
      >
        {lang.language ?? '—'}
